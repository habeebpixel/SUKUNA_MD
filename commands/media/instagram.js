...
function instagramRichCard(profile) {
    ...
    const posts = (profile.posts || []).filter(post => post?.image && post?.shortcode).slice(0, 6).map((post, i) => {
        const postUrl = escapeHtml(`https://instagram.com/p/${post.shortcode}/`);
        return `<a class="post reveal" style="animation-delay:${420 + i * 70}ms" href="${postUrl}" target="_blank" rel="noopener noreferrer" onclick="return openLink('${postUrl}')"><img src="${escapeHtml(post.image)}" alt="Instagram post"><span class="post-icon">📷</span></a>`;
    }).join('');
    const recentPosts = posts ? `<div class="section-title reveal" style="animation-delay:380ms">${escapeHtml(username)}'s recent posts</div><div class="post-grid">${posts}</div>` : '';
    const verifiedBadge = profile.verified ? '<span class="verified">✓</span>' : '';

    return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>
*{box-sizing:border-box}html,body{margin:0;background:#05090b;font-family:Arial,sans-serif}body{padding:0;background:#05090b;color:#f0f4f5}.sheet{min-height:100vh;padding:10px 22px 26px;background:#101a1f;border-radius:28px 28px 0 0;transform:translateY(24px);opacity:0;animation:rise .32s cubic-bezier(.2,.8,.2,1) forwards}.handlebar{width:48px;height:4px;margin:0 auto 25px;border-radius:6px;background:#d8e0e2}.avatar-wrap{text-align:center}.avatar{width:92px;height:92px;border-radius:50%;object-fit:cover;border:3px solid #ee335c;background:#202a2f}.identity{text-align:center;margin-top:10px}.name{font-size:27px;font-weight:400;color:#f4f7f8}.verified{display:inline-flex;align-items:center;justify-content:center;width:19px;height:19px;margin-left:6px;border-radius:50%;background:#2698ee;color:#fff;font-size:12px;font-weight:900;vertical-align:middle}.stats{text-align:center;margin-top:18px;color:#f2f5f6;font-size:16px}.bio{text-align:center;margin-top:18px;color:#e5eaec;font-size:17px;line-height:1.35}.actions{margin-top:22px}.button{display:block;text-align:center;text-decoration:none;padding:17px 10px;border-radius:32px;font-size:17px;font-weight:500;cursor:pointer}.primary{background:#20d268;color:#eefcf2}.description{margin-top:31px;color:#edf2f3;font-size:17px;line-height:1.42}.section-title{margin-top:30px;margin-bottom:12px;color:#9ca8ad;font-size:15px}.post-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;overflow:hidden}.post{position:relative;display:block;height:190px;overflow:hidden;border-radius:8px;background:#1b262b;cursor:pointer}.post img{width:100%;height:100%;object-fit:cover}.post-icon{position:absolute;left:12px;bottom:11px;color:#fff;font-size:16px;text-shadow:0 1px 4px #000}.footer{text-align:center;margin-top:26px;color:#78888f;font-size:12px}.reveal{opacity:0;transform:translateY(10px);animation:fadeUp .32s ease-out forwards}@keyframes rise{to{transform:translateY(0);opacity:1}}@keyframes fadeUp{to{transform:translateY(0);opacity:1}}
</style></head><body><section class="sheet"><div class="handlebar"></div><div class="avatar-wrap"><img class="avatar" src="${escapeHtml(avatar)}" alt="Instagram profile"></div><div class="identity"><span class="name">${name}</span>${verifiedBadge}</div><div class="stats">${stat(profile.postsCount)} posts · ${stat(profile.followers)} followers · ${stat(profile.following || profile.engagement)} ${profile.following ? 'following' : 'engagement'}</div><div class="bio">${escapeHtml(profile.biographyShort || profile.shortBio || profile.fullName || name)}</div><div class="actions"><a class="button primary" id="viewProfileBtn" href="${safeProfileUrl}" target="_blank" rel="noopener noreferrer" onclick="return openLink('${safeProfileUrl}')">📷&nbsp;&nbsp;View profile</a></div><div class="description reveal" style="animation-delay:200ms">${escapeHtml(description)}</div>${recentPosts}<div class="footer reveal" style="animation-delay:600ms">Instagram · SUKUNA MD</div></section>
<script>
function openLink(url){try{window.open(url,'_blank')}catch(e){}try{window.location.href=url}catch(e){}return false}
</script>
</body></html>`;
}
...
// Compact, GUARANTEED-tappable card. This uses WhatsApp's ordinary
// link-preview context (externalAdReply) with the real Instagram URL as the
// message text, so tapping it always opens the real profile in a browser or
// the Instagram app — it does not depend on the experimental rich-HTML
// webview below, which can render a nice snapshot but cannot open external
// links from inside its own sandbox (see the .cursedash/.slot notes on
// FOAHtmlPrimitiveDemoDONOTUSE — it runs local JS fine, but link/window
// navigation out of that webview is not something a bot-forwarded message
// can reliably trigger).
async function sendInstagramLinkPreview({ sock, jid, quoted, profile }) {
    const username = usernameFromInput(profile.username || 'instagram');
    const profileUrl = profile.profileUrl || profile.externalUrl || `https://instagram.com/${encodeURIComponent(username)}`;
    const avatar = profile.avatar || `https://unavatar.io/instagram/${encodeURIComponent(username)}`;
    const title = `${profile.fullName || username}${profile.verified ? ' ✅' : ''}`;
...
// GenAI "Instagram lookup" card — a native IG_PROFILE entity chip inside a
// richResponseMessage/botForwardedMessage. WhatsApp renders a
// GenAISocialEntityItem as a live Instagram profile chip, distinct from the
// HTML card genaiRich.js's sendRichHtml sends. Wired to the already-looked-up
// profile (avatar/full name) instead of the fixed demo values, and sent the
// same way the rest of this codebase sends rich cards.
function buildInstagramGenAILookupContent(username, profile = {}) {
    const displayName = profile.fullName || 'NOT found';
    const avatar = isHttpUrl(profile.avatar) ? profile.avatar : 'https://files.catbox.moe/h87kyf.png';
        ...
        sections: [
            {
                view_model: {
                    primitives: [{
                        ...
                        subtitle: 'By Snøwi',
                        secondary_subtitle: '',
                        image: { url: avatar, mime_type: 'image/png' },
                        entity_id: '123456',
                        entity_url: 't.me/TheSatanicMirror',
                        entity_type: 'WEBSITE',
                        ...
                        { __typename: 'GenAISpacerPrimitive' },
                        {
                            ...
                            inline_entities: [{
                                ...
                                metadata: {
                                    ...
                                    entity_full_name: displayName,
                                    entity_picture_url: avatar,
                                    entity_url: `https://www.instagram.com/${username}`,
...
async function sendInstagramGenAILookup({ sock, jid, quoted, username, profile }) {
    const content = buildInstagramGenAILookupContent(username, profile);
...
module.exports = {
    ...
    async execute({ sock, msg, from, reply, args }) {
        ...
        try {
            ...
            // 1) GenAI "Instagram lookup" chip — the native IG_PROFILE entity
            // card (what you asked to add). Independent of the two cards
            // below; if the recipient's client can't render it, it just
            // doesn't show and the rest of the lookup still works.
            try {
                await sendInstagramGenAILookup({ sock, jid: from, quoted: msg, username, profile });
            } catch (genaiError) {
                console.warn('[insta] GenAI lookup chip failed:', genaiError.message);
            }

            // 2) Compact card built on a real, standard WhatsApp link
            // preview (contextInfo.externalAdReply) around the real
            // instagram.com URL. Tapping this is guaranteed to open the
            // real profile — it does not depend on the experimental
            // webview below.
            try {
                await sendInstagramLinkPreview({ sock, jid: from, quoted: msg, profile });
            } catch (linkError) {
                console.warn('[insta] link preview card failed:', linkError.message);
            }

            // 3) Dark "profile sheet" HTML card for a nicer at-a-glance
            // view (stats, bio, recent posts) — same botForwardedMessage
            // mechanism .slot and .cursedash already use. This is a bonus:
            // all the same info is already visible without tapping
            // anything, and it falls back to a plain button card if the
            // recipient's client can't render it.
            try {
                await sendRichHtml({ sock, jid: from, quoted: msg, html: instagramRichCard(profile) });
            } catch (richError) {
                console.warn('[insta] rich HTML card failed, falling back to button card:', richError.message);
...
module.exports.sendInstagramLinkPreview = sendInstagramLinkPreview;
module.exports.sendInstagramProfileCard = sendInstagramProfileCard;
module.exports.sendInstagramGenAILookup = sendInstagramGenAILookup;
module.exports.buildInstagramGenAILookupContent = buildInstagramGenAILookupContent;
