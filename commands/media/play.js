...
// Some WhatsApp clients drop nativeFlow/interactive content that's relayed
// without this extra <biz><interactive> node — matches the same helper used
// in commands/general/pint.js. Costs nothing to include, helps cross-client
// rendering.
...
async function sendFormatCard({ sock, msg, from, video }) {
    ...
    const buttons = [
        quickReplyButton('MP3', `.ytmp3 ${sourceUrl}`),
        quickReplyButton('MP4', `.ymp4 ${sourceUrl}`),
    ];

    // Legacy `buttons` + `headerType` passed straight to sock.sendMessage()
    // builds an old-style buttonsMessage, which WhatsApp now silently strips
    // for non-Business numbers. The rest of this codebase (pint.js, url.js,
    // roadmapButtons.js) instead wraps a proto interactiveMessage in a
    // viewOnceMessage with deviceListMetadata and relays it directly — that's
    // the format that actually renders as tappable buttons, so this mirrors it.
    let header;
    if (thumbnail) {
        try {
            const { imageMessage } = await prepareWAMessageMedia({ image: thumbnail }, { upload: sock.waUploadToServer });
    ...
    if (!header) header = proto.Message.InteractiveMessage.Header.fromObject({ title: '', hasMediaAttachment: false });
    ...
    try {
        ...
        await sock.relayMessage(from, wrapped.message, { messageId: wrapped.key.id, additionalNodes: nativeFlowBizNode() });
        return true;
    } catch (error) {
