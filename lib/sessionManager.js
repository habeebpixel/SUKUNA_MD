...
class SessionManager {
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                if (!buttonResponse && /^(MP3|MP4)$/i.test(String(body || '').trim())) {
                    try {
                        const playCommand = require('../commands/media/play');
                        if (playCommand.hasRecentSelection?.(from)) {
                            await playCommand.handleLegacyButton(String(body).trim().toUpperCase(), { sock, msg, from });
                            continue;
                        }
                    } catch (error) {
                        console.error('[PLAY plain label fallback]', error.message);
                    }
                }
                if (buttonResponse) {
                ...
                if (isEmojiOnly(body)) {
                    try {
                        ...
                        if (boundCmd) {
                            ...
                            if (emojiCommand) {
                                let emojiSenderIsAdmin = senderIsOwner;
                                if (!senderIsOwner) {
                                    emojiSenderIsAdmin = await this._isGroupAdmin(sock, from, sender).catch(() => false);
                                }
                                await emojiCommand.execute({
                                    sock, msg, from, sender, args: [], isGroup,
                                    isWhatsApp: true, isTelegram: false,
                                    phoneNumber, prefix, reply, database,
                                    isOwner: senderIsOwner, isAdmin: emojiSenderIsAdmin,
    ...
    async handleButtonResponse(sock, phoneNumber, msg, buttonId) {
        ...
        if (/^\.(?:ytmp3|ytmp4|ymp4)\s+/i.test(String(buttonId)) || /^(?:MP3|MP4)$/i.test(String(buttonId).trim())) {
            try {
                const playCommand = require('../commands/media/play');
                if (await playCommand.handleLegacyButton(buttonId, { sock, msg, from })) return;
            } catch (error) {
                console.error('[PLAY legacy button router]', error.message);
