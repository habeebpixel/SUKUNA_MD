...
function extractLegacyPlayButtonResponse(message) {
    ...
    const templateSelected = content?.templateButtonReplyMessage?.selectedId
    ...
    const context = content?.extendedTextMessage?.contextInfo || content?.contextInfo || {};
    const quoted = unwrapIncomingMessage(context.quotedMessage || {});
    if (/^MP[34]$/i.test(String(text || '').trim()) && quoted?.buttonsMessage?.buttons) {
