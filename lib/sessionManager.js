    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Owner Tag: react to every incoming message or sticker from the
                // connected owner or an authorized moderator. This is deliberately
                // placed before command/button routing so the same reaction applies
                // to ordinary chat, media, stickers, and commands without sending
                // a duplicate panel or reply.
                if (!fromMe && (senderIsOwner || this._isModUser(phoneNumber, sender))) {
                    try {
