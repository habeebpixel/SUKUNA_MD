...
const { createMessageContext } = require('../utils/messageContext');
...
const { isEmojiOnly }  = require('../utils/customCommandTriggers');
...
const { setupAntiBot } = require('../utils/antiBotEngine');
const { screenIncoming: screenAntiBug } = require('../utils/antibugEngine');
...
function extractInteractiveButtonResponse(message) {
    ...
    for (const raw of candidates) {
        try {
            ...
            if (typeof raw === 'string' && (raw.startsWith('chroma:') || raw.startsWith('instagram:'))) return raw;
        }
    }
    return null;
}
...
function extractLegacyPlayButtonResponse(message) {
    const content = unwrapIncomingMessage(message);
    const response = findNestedMessagePart(content, 'buttonsResponseMessage');
    const selected = response?.selectedButtonId || response?.selectedDisplayText;
    if (selected) return selected;

    const template = findNestedMessagePart(content, 'templateButtonReplyMessage');
    const templateSelected = template?.selectedId || template?.selectedDisplayText;
    if (templateSelected) return templateSelected;

    // Some older clients send the visible label as a normal quoted message
    // instead of a ButtonsResponseMessage. Only treat it as a play action when
    // the quoted parent is the original buttonsMessage.
    const extended = findNestedMessagePart(content, 'extendedTextMessage');
    const text = content?.conversation || extended?.text;
    const context = extended?.contextInfo || content?.contextInfo || {};
    const quoted = unwrapIncomingMessage(context.quotedMessage || {});
    const anyButtons = findNestedMessagePart(content, 'buttonsMessage');
    if (/^MP[34]$/i.test(String(text || '').trim()) && (quoted?.buttonsMessage?.buttons || anyButtons?.buttons)) {
        return String(text).trim().toUpperCase();
    }
    return null;
}

function findNestedMessagePart(value, key, depth = 0) {
    if (!value || typeof value !== 'object' || depth > 10) return null;
    if (value[key] && typeof value[key] === 'object') return value[key];
    for (const child of Object.values(value)) {
        const found = findNestedMessagePart(child, key, depth + 1);
...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            sock.ev.on('messages.upsert', m => {
                for (const incoming of (m.messages || [])) {
                    const jid = incoming.key?.remoteJid;
                    const id = incoming.key?.id;
                    if (jid && id && incoming.message && !incoming.message.protocolMessage && !incoming.messageStubType) {
                        if (!this._msgCache.has(jid)) this._msgCache.set(jid, new Map());
                        const earlyCache = this._msgCache.get(jid);
                        earlyCache.set(id, incoming);
                        while (earlyCache.size > 500) earlyCache.delete(earlyCache.keys().next().value);
                    }
                }
            ...
            sock.__sukunaMessageCache = this._msgCache;
            sock.__sukunaPhoneNumber = phoneNumber;
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                if (!msg.key || typeof msg.key.remoteJid !== 'string' || !msg.key.remoteJid) continue;

                if (this._isDuplicateMessage(phoneNumber, msg)) continue;

                // Defensive first-pass screening: reject malformed or flood-like
                // messages before view-once recovery, listeners, media downloads,
                // AI, or command execution can touch them.
                const antiBugResult = await screenAntiBug(sock, msg);
                if (antiBugResult.blocked) continue;
                ...
                // read and react with the owner's configured emoji sequence. We bail out of the rest of the pipeline
                            ...
                            if (!seen.has(key)) {
                                ...
                                if (participant) {
                                    await sock.sendMessage(
                                        'status@broadcast',
                                        { react: { text: database.getAutoStatusReaction(phoneNumber).emoji, key: msg.key } },
                ...
                // Owner Tag: react to every message or sticker from the connected
                // owner or an authorized moderator. Include fromMe messages because
                // the linked WhatsApp account is the primary owner identity; the
                // previous !fromMe guard made owner-tag work only for moderators.
                if (senderIsOwner || this._isModUser(phoneNumber, sender)) {
                    try {
                        const ownerTag = database.getOwnerTag(phoneNumber);
                        if (ownerTag?.enabled && ownerTag.emoji && msg.message) {
                            await sock.sendMessage(from, {
                                react: { text: ownerTag.emoji, key: msg.key },
                            }).catch(() => {});
                        }
                    } catch (_) {}
                }
                ...
                // Legacy buttonsMessage replies are commonly wrapped by
                // WhatsApp in ephemeral/view-once containers. Inspect the
                // unwrapped message first or MP3/MP4 taps fall through as
                // ordinary messages and never reach the play downloader.
                const buttonContent = unwrapIncomingMessage(msg);
                const buttonResponse =
                    extractLegacyPlayButtonResponse(msg) ||
                    extractInteractiveButtonResponse({ ...msg, message: buttonContent });
                if (buttonResponse) {
                            ...
                            if (id) {
                                ...
                                if (boundCmd) {
                                    ...
                                    if (stickerCommand) {
                                        ...
                                        let senderIsAdmin = senderIsOwner || senderIsMod;
                                        if (isGroup && !senderIsOwner) {
                                            senderIsAdmin = senderIsMod || await this._isGroupAdmin(sock, from, sender).catch(() => false);
                                        }
                                        const privateVvp = boundCmd === 'vvp' && senderIsMod && !this._isSudoUser(phoneNumber, sender);
                                        const triggerFrom = privateVvp ? sender : from;
                                        const triggerM = createMessageContext({
                                            msg, sock, from, sender, reply, args: [], prefix,
                                            commandName: boundCmd, forwardTo: triggerFrom,
                                        });
                                        const stickerCtx = {
                                            sock, msg, from: triggerFrom, sender,
                ...
                // ── Emoji Custom Command Trigger ─────────────────────────────
                // Emoji bindings are exact bot-global matches. Reply to an
                // emoji with .setcmd <command> to create one.
                if (isEmojiOnly(body)) {
                    try {
                        const emojiKey = body.trim();
                        const boundCmd = database.getEmojiCmd(from, emojiKey);
                        if (boundCmd) {
                            const emojiCommand = commandLoader.getCommand(boundCmd);
                            if (emojiCommand) {
                                const emojiSenderIsMod = this._isModUser(phoneNumber, sender);
                                let emojiSenderIsAdmin = senderIsOwner || emojiSenderIsMod;
                                if (!senderIsOwner) {
                                    emojiSenderIsAdmin = emojiSenderIsMod || await this._isGroupAdmin(sock, from, sender).catch(() => false);
                                }
                                const privateVvp = boundCmd === 'vvp' && emojiSenderIsMod && !this._isSudoUser(phoneNumber, sender);
                                const triggerFrom = privateVvp ? sender : from;
                                const triggerM = createMessageContext({
                                    msg, sock, from, sender, reply, args: [], prefix,
                                    commandName: boundCmd, forwardTo: triggerFrom,
                                });
                                await emojiCommand.execute({
                                    sock, msg, from: triggerFrom, sender, args: [], isGroup,
                                    isWhatsApp: true, isTelegram: false,
                                    phoneNumber, prefix, reply, database,
                                    isOwner: senderIsOwner || emojiSenderIsMod, isAdmin: emojiSenderIsAdmin,
                                    isMod: emojiSenderIsMod,
                                    m: triggerM, quoted: triggerM.quoted,
                                });
                            } else {
                                database.deleteGlobalEmojiCmd(emojiKey);
                            }
                            continue;
                        }
                    } catch (e) { console.error('[EMOJI CMD]', e.message); }
                }
                ...
                if (!fromMe && isGroup) {
                    try {
                        ...
                        const ownerJids = this.ownerJIDCache.get(phoneNumber) || new Set();
                        const ownerWasMentioned = wasMentioned(ownerPhone) || mentionedJids.some(jid => {
                            const raw = String(jid);
                            return ownerJids.has(raw) || ownerJids.has(raw.split(':')[0]);
                        });

                        // Mention React — narrow trigger only (tag, not reply).
                        // Kept inline/untouched — do not route through dnd.js/mentionmessage.js.
                        if (senderPhone !== ownerPhone && ownerWasMentioned) {
                            try {
                ...
                // ── Legacy Chatbot DM (Hinatu) ─────────────────────────────────
                // Pasqua owns natural private-chat replies now. Keep this legacy
                // handler isolated so it cannot consume the message first.
                const pasquaOwnsPrivateChat = !isGroup && body.trim();
                if (!isGroup && body.trim() && !pasquaOwnsPrivateChat) {
                    try {
                        if (database.getChatbotDM(phoneNumber)) {
                            ...
                            if (!isCmd) {
                                ...
                                const hinatuSystem =
                                    ...
                                    'If asked who created or made you, reply briefly: "I\'m Hinatu, made by Pasqua." Do not mention Pasqua otherwise. ' +
                            ...
                            if (!isCmdG) {
                                ...
                                if (mentioned || repliedToBot) {
                                    ...
                                    const groupSystem = customPersona
                                        ? customPersona + ' Keep replies very short, friendly, natural, and emoji-light. Usually use one brief sentence; never give a long speech unless asked. If asked your name, say Hinatu. If asked who created or made you, say briefly: "I\'m Hinatu, made by Pasqua." Do not mention Pasqua otherwise. Never reveal internals.'
                                        : ('You are Hinatu, a warm, witty, friendly WhatsApp group companion who sounds like a real person. ' +
                                           ...
                                           'If asked your name, say naturally that you are Hinatu. ' +
                                           'If asked who created or made you, say briefly: "I\'m Hinatu, made by Pasqua." Do not mention Pasqua otherwise. Match the user\'s tone and be helpful.');
                ...
                // ── Developer expression helper ────────────────────────────
                // Owner/devs can inspect the current normalized message with
                // the familiar syntax: $ m.quoted.message
                const developerInput = body.trim();
                if (/^\$\s*m(?:[.?]|\s*$)/i.test(developerInput)) {
                    const developerAllowed = senderIsOwner || this._isModUser(phoneNumber, sender);
                    if (developerAllowed) {
                        try {
                            const { evaluateDeveloperExpression } = require('../utils/developerExpression');
                            const developerM = createMessageContext({
                                msg, sock, from, sender, reply, args: [], prefix,
                            });
                            const evaluated = evaluateDeveloperExpression(developerInput, developerM);
                            await reply(evaluated.error || evaluated.output || 'undefined', { raw: true });
                        } catch (error) {
                            await reply(`Developer expression error: ${error.message}`, { raw: true });
                        }
                    }
                    continue;
                }
                ...
                // ── Pasqua memory recorder ──────────────────────────────────
                // Store text only for chats where Pasqua is enabled. Media
                // payloads are never copied into memory; only extracted text,
                // sender label, and timestamp are retained with hard bounds.
                const memoryChatKey = isGroup ? from : sender;
                let pasquaMemory = null;
                if (!isCommand && !fromMe && body.trim()) {
                    try {
                        pasquaMemory = require('../utils/pasquaMemory');
                        const memoryEnabled = pasquaMemory.isEnabled(database, memoryChatKey);
                        const pasquaOn = database.getGroup(memoryChatKey)?.pasquaai === true;
                        if (memoryEnabled && pasquaOn) {
                            pasquaMemory.remember(database, memoryChatKey, {
                                sender,
                                senderLabel: msg.pushName || sender,
                                text: body.trim(),
                                fromMe: false,
                            });
                        }
                    } catch (memoryError) {
                        console.error('[PASQUA MEMORY]', memoryError.message);
                    }
                }
                ...
                // ── Pasqua AI — conversational mode ─────────────────────────
                // DMs chat naturally; enabled groups receive Pasqua replies freely
                // without requiring a mention, reply, or name trigger. An attached
                // photo/video is also a Pasqua prompt, so no `.pasqua` prefix is
                // needed once Pasqua is enabled in the chat.
                const hasPasquaMedia = Boolean(
                    messageContent?.imageMessage
                    || messageContent?.videoMessage
                    || messageContent?.viewOnceMessage?.message?.imageMessage
                    || messageContent?.viewOnceMessage?.message?.videoMessage
                    || messageContent?.viewOnceMessageV2?.message?.imageMessage
                    || messageContent?.viewOnceMessageV2?.message?.videoMessage
                ...
                if (!isCommand && (body.trim() || hasPasquaMedia)) {
                    try {
                        ...
                        if (chatData?.pasquaai) {
                            ...
                            let pasquaMedia = null;
                            if (hasPasquaMedia) {
                                try {
                                    pasquaMedia = await require('../utils/pasquaMedia').extractPasquaMedia(msg);
                                } catch (mediaError) {
                                    console.error('[Pasqua media]', mediaError.message);
                                    await reply(`I could not read that media: ${mediaError.message}`, { raw: true });
                                    continue;
                                }
                            }
                            const mediaPrompt = pasquaMedia?.type === 'video'
                            ...
                            const trigger = pasquaMedia && !isGroup
                                ? { triggered: true, repliedToBot: false, nameCalled: false, text: body.trim() || mediaPrompt }
                                : detectTrigger({
                                    body,
                                    content: messageContent,
                                    botIds: this._botIds(sock),
                                    normalizeJid: jid => this._normJid(jid),
                                    conversationKey: 'pasqua:' + phoneNumber + ':' + chatKey,
                                    isPrivate: !isGroup,
                                    allowNaturalChat: true,
                                });
                            if (!trigger.triggered) continue;
                            if (isGroup && !trigger.repliedToBot && !trigger.nameCalled) continue;

                            const routed = routeNaturalLanguage(trigger.text);
                            if (routed?.kind === 'pair-link') {
                                await reply(
                                    '🔗 Pair Server 1: https://pair-site-wmte.onrender.com\n' +
                                    '🔗 Pair Server 2 (backup): https://pair-site-91ob.onrender.com\n\n' +
                                    'Use Server 2 if Server 1 is unavailable.\n\n' +
                                    '1. Open either link and enter your WhatsApp number.\n' +
                                    '2. Copy SESSION_ID and PAIR_NUMBER.\n' +
                                    '3. Add both to your host, then deploy and start the bot.',
                                    { raw: true }
                                );
                                continue;
                            }
                            ...
                            const typingDelay = Math.min(1800, Math.max(650, trigger.text.length * 18));
                            await sock.sendPresenceUpdate('composing', from).catch(() => {});
                            await new Promise(resolve => setTimeout(resolve, typingDelay));
                            let pasquaError = null;
                            const aiReply = pasquaMedia
                                ? await require('../utils/smartAI').askMultimodal({
                                    key: 'pasqua:' + phoneNumber + ':' + chatKey,
                                    system: 'You are Pasqua, the sharp, concise AI personality of SUKUNA MD. Analyze attached photos or sampled video frames accurately. State what is directly visible, distinguish uncertainty, and never invent details.',
                                    user: trigger.text,
                                    media: pasquaMedia.media,
                                    remember: true,
                                    compact: true,
                                }).catch(error => {
                                    pasquaError = error;
                                    console.error('[PasquaAI] vision provider failed:', error.message);
                                    return null;
                                })
                                : await getPasquaAIReply(
                                    concisePrompt(trigger.text, knowledge),
                                    'pasqua:' + phoneNumber + ':' + chatKey,
                                    { memoryContext: pasquaMemory?.getContext(database, chatKey) },
                                ).catch(error => {
                                pasquaError = error;
                            ...
                            if (aiReply) {
                                await reply(aiReply, { raw: true });
                            } else {
                                const { getAIStatus } = require('../utils/smartAI');
                                const aiStatus = getAIStatus();
                                const failure = aiStatus.lastError || pasquaError;
                                let detail = '';
                                if (!aiStatus.configured && aiStatus.primary === 'agnes') {
                                    detail = ' Agnes is not configured: add your key to `utils/smartAI.js` in `AGNES_API_KEY_FILE`.'.replace(/`/g, '');
                                } else if (failure?.status === 401) {
                                    detail = ' Agnes rejected the API key (HTTP 401); verify the key in `AGNES_API_KEY_FILE`.';
                                } else if (failure?.status === 429) {
                                    detail = ' Agnes rate-limited the request (HTTP 429); wait briefly and try again.';
                                } else if (failure?.status >= 500) {
                                    detail = ` Agnes returned a temporary server error (HTTP ${failure.status}); try again shortly.`;
                                } else if (failure?.message) {
                                    detail = ` Agnes error: ${String(failure.message).slice(0, 180)}.`;
                                }
                                await reply(`I’m here, but Agnes AI did not return a reply.${detail}`, { raw: true });
                            }
                        }
                    } catch (e) {
                ...
                // Rich-response commands inspect this flag and automatically
                // send readable plain text when the deployment is in iPhone
                // compatibility mode.
                sock.__sukunaDeviceMode = database.getDeviceMode();
                ...
                // ── Optional command reaction ─────────────────────────────────
                // React once before every permitted command, then let the command
                // send its normal reply. The toggle is persisted per bot session.
                if (database.getCmdReact(phoneNumber)) {
                    try {
                        this._cmdReactionIndex = (this._cmdReactionIndex || 0) + 1;
                        const emoji = CMD_REACTION_EMOJIS[
                            (this._cmdReactionIndex - 1) % CMD_REACTION_EMOJIS.length
                        ];
                        await sock.sendMessage(from, {
    ...
    async handleButtonResponse(sock, phoneNumber, msg, buttonId) {
        ...
        if (String(buttonId).startsWith('chromatic:')) {
            try {
                const chromaticCommand = require('../commands/expansion/chromatic');
                const handled = await chromaticCommand.handleButton(buttonId, { sock, msg, from });
                if (handled) return;
            } catch (error) {
                console.error('[CHROMATIC button router]', error.message);
                await reply('❌ Chromatic control action failed. Run `.chromatic` again.');
                return;
            }
        }

        if (String(buttonId).startsWith('play:')) {
            try {
                const playCommand = require('../commands/media/play');
                const handled = await playCommand.handleButton(buttonId, { sock, msg, from });
                if (handled) return;
            } catch (error) {
                console.error('[PLAY button router]', error.message);
                await reply('❌ Play download action failed. Run `.play` again.');
                return;
            }
        }
        ...
        if (String(buttonId).startsWith('stream_details:')) {
            try {
                const streamSearch = require('../commands/media/streamsearch');
                if (typeof streamSearch.handleButton === 'function') {
                    await streamSearch.handleButton(String(buttonId), { sock, msg, from });
                    return;
                }
            } catch (error) {
                console.error('[STREAM SEARCH details]', error.message);
                await reply('❌ Search details could not be opened. Run .sch again.');
                return;
            }
        }
        if (String(buttonId).startsWith('instagram:profile:')) {
            const username = String(buttonId).slice('instagram:profile:'.length).trim().toLowerCase();
            try {
                const instagramCommand = require('../commands/media/instagram');
                let profile;
                try { profile = await instagramCommand.lookupInstagram(username); }
                catch (_) { profile = await instagramCommand.lookupInstagramUser(username); }
                await instagramCommand.sendInstagramProfileCard({ sock, jid: from, quoted: msg, profile });
            } catch (error) {
                console.error('[INSTAGRAM profile CTA]', error.message);
                await reply(`❌ Could not load live Instagram results for @${username}.`);
            }
            return;
        }

        if (String(buttonId).startsWith('instagram:posts:')) {
            const username = String(buttonId).slice('instagram:posts:'.length).trim().toLowerCase();
            try {
                const instagramCommand = require('../commands/media/instagram');
                let profile;
                try { profile = await instagramCommand.lookupInstagram(username); }
                catch (_) { profile = await instagramCommand.lookupInstagramUser(username); }
                await instagramCommand.sendInstagramPosts({ sock, jid: from, quoted: msg, profile });
            } catch (error) {
                console.error('[INSTAGRAM posts CTA]', error.message);
                await reply(`❌ Could not load recent posts for @${username}.`);
            }
            return;
        }
